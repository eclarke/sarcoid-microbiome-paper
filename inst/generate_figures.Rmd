...
StandardizeMetadata <- function(ds) {
  ...
  if (ds$datatype == "virome")
    new.s$library_selection <- ifelse(
      s$LibraryType == "Genomiphi", "MDA", "RT-PCR"
  )
...
combined <- do.call(rbind, lapply(list(a.16s, a.its, b.16s, b.its, c.16s, c.its, c.vir, de.16s, de.its, de.wgs), StandardizeMetadata))
write.table(combined, file=file.path())
```
