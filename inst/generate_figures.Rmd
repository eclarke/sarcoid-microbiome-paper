...
invisible(within(a.16s, {
  ...
  heatmap.data <- agg %>%
    filter(SampleType %in% c("lymph_node", "paraffin")) %>%
    SarcoidMicrobiome:::MakeHeatmapData(s, min.samples = 1, min.rank.1 = "Genus") %>%
...
invisible(within(a.16s, {
  ...
  ggsave(file.path(
    opts$figure_fp, 
    sprintf("FigXa_Set%s_%s_SummaryDynamitePlots.pdf", sampleset, datatype)),
...
invisible(within(b.its, {
  p <- agg %>%
    ...
    PlotTopBarcharts() +
    scale_fill_manual("", values=top.barchart.fills) +
    scale_y_continuous(limits=c(0,.25), expand=c(0,0), labels=scales::percent) +
    labs(
      y="Median abundance", 
      title=sprintf("Dominant taxa in Set %s (%s)", sampleset, datatype))
...
invisible(within(c.its, {
  p <- agg %>%
    ...
    filter(
      SampleType %in% c("BAL", "prewash"), 
      StudyGroup %in% c("healthy", "sarcoidosis")) %>%
    MakeTopBarchartData(s, top=6, min.rank="Family") %>%
    PlotTopBarcharts() +
    scale_fill_manual("", values=top.barchart.fills) +
    labs(
      y="Median abundance", 
...
invisible(within(a.its, {
  ...
  PCoAChunk(
    .agg, s, tree, rarefy.depth=900, 
    testing.terms=c("readcount", "StudyGroup"), 
    sampleset="A", dataset="ITS", subset="paraffin", fig.num="S2C", fig.dir=opts$figure_fp)
}))
```
...
invisible(within(a.16s, {
  ...
  PCoAChunk(
    .agg, s, tree, rarefy.depth=1000, 
    testing.terms=c("readcount", "StudyGroup"),
    sampleset=sampleset, dataset=datatype, subset="paraffin", fig.num="S2D", fig.dir=opts$figure_fp)
}))
```
